...
sudo cp /usr/local/bin/Change\ LED\ to\ Blue.sh /opt/system/.
...
sudo rm /opt/system/Change\ LED\ to\ Blue.sh
printf "\033c" >> /dev/tty1
sudo systemctl restart emulationstation
EOF
...
sleep 0.2
cp "$BIN/Change LED to Red.sh" "${OPT}/."
...
chmod +x "$OPT/Change LED to Red.sh"
